...
public class ConfigurationTest {
	...
	private final String noServiceAddressMessage = "You must specify service_address parameter";
	private final String noTsdbAddressMessage = "You must specify tsdb_address parameter";
	...
	@Test
	public void testNoTsdbAddressParameterSet(){
		String [] args = new String[]{"mode=LOAD","metric=agh"};
		Configuration config = new Configuration(args);
		assertEquals(false, config.isValid());
		assertEquals(noTsdbAddressMessage, config.getErrorMsg());
	}
	
	@Test
	public void testNoTagsSetInGenerateMode(){
		String [] args = new String[]{"mode=GENERATE","metric=agh","tsdb_address=localhost:2002"};
		Configuration config = new Configuration(args);
		assertEquals(false, config.isValid());
	...
	public void testNoFileSetInLoadMode(){
		String [] args = new String[]{"mode=LOAD","metric=agh","tsdb_address=localhost:2002",
									  "tags=cpu:01"};
		Configuration config = new Configuration(args);
		assertEquals(false, config.isValid());
	...
	@Test
	public void testNoServiceAddressMessage(){
		String [] args = new String[]{"mode=TEST1","metric=agh","tsdb_address=localhost:2002",
				"tags=cpu:01","req_nr=100","file=data.txt"};
		Configuration config = new Configuration(args);
		assertEquals(false, config.isValid());
		assertEquals(noServiceAddressMessage, config.getErrorMsg());
	}

	@Test
	public void testNoAggregatorSetInLoadMode(){
		String [] args = new String[]{"mode=TEST1","metric=agh","tsdb_address=localhost:2002",
									  "tags=cpu:01","req_nr=100","file=data.txt", "service_address=127.0.0.1"};
		Configuration config = new Configuration(args);
		assertEquals(false, config.isValid());
